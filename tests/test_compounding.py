...
def test_doubled_equity_doubles_dollar_caps() -> None:
    ...
    assert scaled.options.max_contract_cost_usd == 50.0
    assert scaled.portfolio.max_total_premium_at_risk_usd == 120.0
    assert scaled.portfolio.daily_loss_stop_usd == 20.0
...
def test_losses_shrink_caps_but_drawdown_keeps_peak_reference() -> None:
    ...
    # hard drawdown stop stays referenced to the peak (still $25).
    scaled = _mandate().scaled_for_equity(80.0, 100.0)
    assert scaled.options.max_contract_cost_usd == 20.0
    assert scaled.portfolio.daily_loss_stop_usd == 8.0
    assert scaled.portfolio.hard_drawdown_stop_usd == 25.0
...
def test_scale_floor_prevents_degenerate_caps() -> None:
    ...
    assert scaled.options.max_contract_cost_usd == 2.5
    assert scaled.portfolio.daily_loss_stop_usd == 1.0
